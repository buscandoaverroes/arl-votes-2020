# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-19-oct.R
# imports and processes aggregated vote info from Arlington for 19th october
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #


# load + process voting data file ---- 
# page info 
raw.info <- pdf_info(file.path(votes, "10-19-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "10-19-2020.pdf"))
...
vote19 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data19 <-
  left_join(vote19, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
vote.data19 <-
  ...
  mutate(
    ...
    date     = lubridate::as_date(ymd("2020-10-19"))
  )

# check math!
assert_that( sum(vote.data19$mail.outstanding.pct) >= 99.5 & sum(vote.data19$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data19$early.voted.pct) >= 99.5 & sum(vote.data19$early.voted.pct) <= 100.5)
assert_that( sum(vote.data19$mail.received.pct) >= 99.5 & sum(vote.data19$mail.received.pct) <= 100.5)
assert_that( sum(vote.data19$prec.tot.votes.pct) >= 99.5 & sum(vote.data19$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data19$prec.early.turnout + vote.data19$prec.mail.turnout) -
                    vote.data19$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Mail.Outstanding` = sum(vote.data19$`Mail.Outstanding`),
  `Mail.Received` = sum(vote.data19$`Mail.Received`),
  `Early.Voted` = sum(vote.data19$`Early.Voted`),
  `Active.Registered` = sum(vote.data19$`Active.Registered`),
  `Inactive.Registered` = sum(vote.data19$`Inactive.Registered`),
  `All.Registered` = sum(vote.data19$`All.Registered`),
  Military = sum(vote.data19$Military),
  Overseas = sum(vote.data19$Overseas),
  Federal = sum(vote.data19$Federal),
  `Total.Votes` = sum(vote.data19$`Total.Votes`),
  `Outstanding.Votes` = sum(vote.data19$`Outstanding.Votes`),
  `Percent.Outstanding.Votes` = sum(vote.data19$`Percent.Outstanding.Votes`),
  `Mail.Ballots.Requested` = sum(vote.data19$`Mail.Ballots.Requested`),
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data19$`Mail.Ballots.Counted`) / sum(vote.data19$`Mail.Received`)), 1),
  `Precinct.Share.of.All.Votes` =  100, # must be 100
  `Mail.Ballot.Return.Rate` = round( (sum(vote.data19$`Mail.Received`) /
                                        sum(vote.data19$`Mail.Ballots.Requested`)), 2),
  ...
  `Mail.Ballots.Counted`  = sum(vote.data19$`Mail.Ballots.Counted`),
  date      = lubridate::as_date(ymd("2020-10-19"))
)

## append arl totals to vote.data 
vote.data19 <- bind_rows(vote.data19, arl)
...
assert_that(sum(arl$`Mail.Ballots.Requested`[1] + e.totmail19) == totals.tib$total.mail[1]) # 101 undercount
...
assert_that((sum(arl$`Mail.Outstanding`[1]) + e.outstanding19) == totals.tib$mailed[1]) # 101 off undercount

# assert that there are 54 precincts
assert_that(nrow(vote.data19) == nrow )
