# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-26-oct.R
# imports and processes aggregated vote info from Arlington for 26th october
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #


# load + process voting data file ---- 
# page info 
raw.info <- pdf_info(file.path(votes, "10-26-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "10-26-2020.pdf"))
...
vote26 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data26 <-
  left_join(vote26, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
vote.data26 <-
  ...
  mutate(
    ...
    date     = lubridate::as_date(ymd("2020-10-26"))
  )

# check math!
assert_that( sum(vote.data26$mail.outstanding.pct) >= 99.5 & sum(vote.data26$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data26$early.voted.pct) >= 99.5 & sum(vote.data26$early.voted.pct) <= 100.5)
assert_that( sum(vote.data26$mail.received.pct) >= 99.5 & sum(vote.data26$mail.received.pct) <= 100.5)
assert_that( sum(vote.data26$prec.tot.votes.pct) >= 99.5 & sum(vote.data26$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data26$prec.early.turnout + vote.data26$prec.mail.turnout) -
                    vote.data26$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Mail.Outstanding` = sum(vote.data26$`Mail.Outstanding`),
  `Mail.Received` = sum(vote.data26$`Mail.Received`),
  `Early.Voted` = sum(vote.data26$`Early.Voted`),
  `Active.Registered` = sum(vote.data26$`Active.Registered`),
  `Inactive.Registered` = sum(vote.data26$`Inactive.Registered`),
  `All.Registered` = sum(vote.data26$`All.Registered`),
  Military = sum(vote.data26$Military),
  Overseas = sum(vote.data26$Overseas),
  Federal = sum(vote.data26$Federal),
  `Total.Votes` = sum(vote.data26$`Total.Votes`),
  `Outstanding.Votes` = sum(vote.data26$`Outstanding.Votes`),
  `Percent.Outstanding.Votes` = sum(vote.data26$`Percent.Outstanding.Votes`),
  `Mail.Ballots.Requested` = sum(vote.data26$`Mail.Ballots.Requested`),
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data26$`Mail.Ballots.Counted`) / sum(vote.data26$`Mail.Received`)), 1),
  `Precinct.Share.of.All.Votes` =  100, # must be 100
  `Mail.Ballot.Return.Rate` = round( (sum(vote.data26$`Mail.Received`) /
                                        sum(vote.data26$`Mail.Ballots.Requested`)), 2),
  ...
  `Mail.Ballots.Counted`  = sum(vote.data26$`Mail.Ballots.Counted`),
  date      = lubridate::as_date(ymd("2020-10-26"))
)

## append arl totals to vote.data 
vote.data26 <- bind_rows(vote.data26, arl)
...
assert_that(sum(arl$`Mail.Ballots.Requested`[1] + e.totmail26) == totals.tib$total.mail[1]) # 101 undercount
...
assert_that((sum(arl$`Mail.Outstanding`[1]) + e.outstanding26) == totals.tib$mailed[1]) # 101 off undercount

# assert that there are 54 precincts
assert_that(nrow(vote.data26) == nrow )
