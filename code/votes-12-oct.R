# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-12-oct.R
# imports and processes aggregated vote info from Arlington for 12th october
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
...
# import both pages
raw1 <- pdf_text(file.path(votes, "arl-votes-10-12.pdf"))
...
vote12 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data12 <-
  left_join(vote12, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
# check math!
assert_that( sum(vote.data12$mail.outstanding.pct) >= 99.5 & sum(vote.data12$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data12$early.voted.pct) >= 99.5 & sum(vote.data12$early.voted.pct) <= 100.5)
assert_that( sum(vote.data12$mail.received.pct) >= 99.5 & sum(vote.data12$mail.received.pct) <= 100.5)
assert_that( sum(vote.data12$prec.tot.votes.pct) >= 99.5 & sum(vote.data12$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data12$prec.early.turnout + vote.data12$prec.mail.turnout) -
                    vote.data12$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data12$`Mail.Ballots.Counted`) / sum(vote.data12$`Mail.Received`)), 1),
