# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-27-oct.R
# imports and processes aggregated vote info from Arlington for 27th october
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #


# load + process voting data file ---- 
# page info 
raw.info <- pdf_info(file.path(votes, "10-27-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "10-27-2020.pdf"))
...
vote27 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data27 <-
  left_join(vote27, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
vote.data27 <-
  ...
  mutate(
    ...
    date     = lubridate::as_date(ymd("2020-10-27"))
  )

# check math!
assert_that( sum(vote.data27$mail.outstanding.pct) >= 99.5 & sum(vote.data27$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data27$early.voted.pct) >= 99.5 & sum(vote.data27$early.voted.pct) <= 100.5)
assert_that( sum(vote.data27$mail.received.pct) >= 99.5 & sum(vote.data27$mail.received.pct) <= 100.5)
assert_that( sum(vote.data27$prec.tot.votes.pct) >= 99.5 & sum(vote.data27$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data27$prec.early.turnout + vote.data27$prec.mail.turnout) -
                    vote.data27$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Mail.Outstanding` = sum(vote.data27$`Mail.Outstanding`),
  `Mail.Received` = sum(vote.data27$`Mail.Received`),
  `Early.Voted` = sum(vote.data27$`Early.Voted`),
  `Active.Registered` = sum(vote.data27$`Active.Registered`),
  `Inactive.Registered` = sum(vote.data27$`Inactive.Registered`),
  `All.Registered` = sum(vote.data27$`All.Registered`),
  Military = sum(vote.data27$Military),
  Overseas = sum(vote.data27$Overseas),
  Federal = sum(vote.data27$Federal),
  `Total.Votes` = sum(vote.data27$`Total.Votes`),
  `Outstanding.Votes` = sum(vote.data27$`Outstanding.Votes`),
  `Percent.Outstanding.Votes` = sum(vote.data27$`Percent.Outstanding.Votes`),
  `Mail.Ballots.Requested` = sum(vote.data27$`Mail.Ballots.Requested`),
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data27$`Mail.Ballots.Counted`) / sum(vote.data27$`Mail.Received`)), 1),
  `Precinct.Share.of.All.Votes` =  100, # must be 100
  `Mail.Ballot.Return.Rate` = round( (sum(vote.data27$`Mail.Received`) /
                                        sum(vote.data27$`Mail.Ballots.Requested`)), 2),
  ...
  `Mail.Ballots.Counted`  = sum(vote.data27$`Mail.Ballots.Counted`),
  date      = lubridate::as_date(ymd("2020-10-27"))
)

## append arl totals to vote.data 
vote.data27 <- bind_rows(vote.data27, arl)
...
assert_that(sum(arl$`Mail.Ballots.Requested`[1] + e.totmail27) == totals.tib$total.mail[1]) # 101 undercount
...
assert_that((sum(arl$`Mail.Outstanding`[1]) + e.outstanding27) == totals.tib$mailed[1]) # 101 off undercount

# assert that there are 54 precincts
assert_that(nrow(vote.data27) == nrow )
