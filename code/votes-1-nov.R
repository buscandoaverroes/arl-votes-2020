# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-1-nov.R
# imports and processes aggregated vote info from Arlington for 1st November
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #


# load + process voting data file ---- 
# page info 
raw.info <- pdf_info(file.path(votes, "11-1-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "11-1-2020.pdf"))
...
vote1 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
vote.data1 <-
  ...
  mutate(
    ...
    date     = lubridate::as_date(ymd("2020-11-1"))
  )

# check math!
assert_that( sum(vote.data1$mail.outstanding.pct) >= 99.5 & sum(vote.data1$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data1$early.voted.pct) >= 99.5 & sum(vote.data1$early.voted.pct) <= 100.5)
assert_that( sum(vote.data1$mail.received.pct) >= 99.5 & sum(vote.data1$mail.received.pct) <= 100.5)
assert_that( sum(vote.data1$prec.tot.votes.pct) >= 99.5 & sum(vote.data1$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data1$prec.early.turnout + vote.data1$prec.mail.turnout) -
                    vote.data1$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Mail.Outstanding` = sum(vote.data1$`Mail.Outstanding`),
  `Mail.Received` = sum(vote.data1$`Mail.Received`),
  `Early.Voted` = sum(vote.data1$`Early.Voted`),
  `Active.Registered` = sum(vote.data1$`Active.Registered`),
  `Inactive.Registered` = sum(vote.data1$`Inactive.Registered`),
  `All.Registered` = sum(vote.data1$`All.Registered`),
  Military = sum(vote.data1$Military),
  Overseas = sum(vote.data1$Overseas),
  Federal = sum(vote.data1$Federal),
  `Total.Votes` = sum(vote.data1$`Total.Votes`),
  `Outstanding.Votes` = sum(vote.data1$`Outstanding.Votes`),
  `Percent.Outstanding.Votes` = sum(vote.data1$`Percent.Outstanding.Votes`),
  `Mail.Ballots.Requested` = sum(vote.data1$`Mail.Ballots.Requested`),
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data1$`Mail.Ballots.Counted`) / sum(vote.data1$`Mail.Received`)), 1),
  `Precinct.Share.of.All.Votes` =  100, # must be 100
  `Mail.Ballot.Return.Rate` = round( (sum(vote.data1$`Mail.Received`) /
                                        sum(vote.data1$`Mail.Ballots.Requested`)), 2),
  ...
  `Mail.Ballots.Counted`  = sum(vote.data1$`Mail.Ballots.Counted`),
  date      = lubridate::as_date(ymd("2020-11-1"))
)

## append arl totals to vote.data 
vote.data1 <- bind_rows(vote.data1, arl)
...
assert_that(sum(arl$`Mail.Ballots.Requested`[1] + e.totmail1) == totals.tib$total.mail[1]) # 101 undercount
...
assert_that((sum(arl$`Mail.Outstanding`[1]) + e.outstanding1) == totals.tib$mailed[1]) # 101 off undercount

# assert that there are 54 precincts
assert_that(nrow(vote.data1) == nrow )
