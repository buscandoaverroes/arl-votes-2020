# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-21-oct.R
# imports and processes aggregated vote info from Arlington for 21th october
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #

# note that I will not count 21 october becauset there is seems to be a 
# problem with importing the raw document, so I will skip this date.
# The issue is that there is a blank row on page 1 that has no obvious precinct 
# name, but does have voting values, which, because of the 'no-name' precinct, 
# cannot be attributed to any voting precicint....

# load + process voting data file ---- 
# page info 
raw.info <- pdf_info(file.path(votes, "10-21-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "10-21-2020.pdf"))
...
pg1.tib <- tibble(RawText=pg1.split) %>%
  filter(row_number() <= max1) %>% # we need to include up through row 46 because we account for extra blank row
  filter(row_number() >= min1) %>% 
  filter(row_number() != 28) %>% # row 27 is a blank
  mutate(TextSquish=str_replace_all(RawText, "(\\.\\s)", "\\.")) %>%
  separate(TextSquish, into=c('precinct', 'Rest'), sep = "-",
           extra="merge") %>%
  mutate(Rest=str_squish(Rest),
...
vote21 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data21 <-
  left_join(vote21, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
vote.data21 <-
  ...
  mutate(
    ...
    date     = lubridate::as_date(ymd("2020-10-21"))
  )

# check math!
assert_that( sum(vote.data21$mail.outstanding.pct) >= 99.5 & sum(vote.data21$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data21$early.voted.pct) >= 99.5 & sum(vote.data21$early.voted.pct) <= 100.5)
assert_that( sum(vote.data21$mail.received.pct) >= 99.5 & sum(vote.data21$mail.received.pct) <= 100.5)
assert_that( sum(vote.data21$prec.tot.votes.pct) >= 99.5 & sum(vote.data21$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data21$prec.early.turnout + vote.data21$prec.mail.turnout) -
                    vote.data21$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Mail.Outstanding` = sum(vote.data21$`Mail.Outstanding`),
  `Mail.Received` = sum(vote.data21$`Mail.Received`),
  `Early.Voted` = sum(vote.data21$`Early.Voted`),
  `Active.Registered` = sum(vote.data21$`Active.Registered`),
  `Inactive.Registered` = sum(vote.data21$`Inactive.Registered`),
  `All.Registered` = sum(vote.data21$`All.Registered`),
  Military = sum(vote.data21$Military),
  Overseas = sum(vote.data21$Overseas),
  Federal = sum(vote.data21$Federal),
  `Total.Votes` = sum(vote.data21$`Total.Votes`),
  `Outstanding.Votes` = sum(vote.data21$`Outstanding.Votes`),
  `Percent.Outstanding.Votes` = sum(vote.data21$`Percent.Outstanding.Votes`),
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data21$`Mail.Ballots.Counted`) / sum(vote.data21$`Mail.Received`)), 1),
  `Mail.Ballots.Requested` = sum(vote.data21$`Mail.Ballots.Requested`),
  `Precinct.Share.of.All.Votes` =  100, # must be 100
  `Mail.Ballot.Return.Rate` = round( (sum(vote.data21$`Mail.Received`) /
                                        sum(vote.data21$`Mail.Ballots.Requested`)), 2),
  ...
  `Mail.Ballots.Counted`  = sum(vote.data21$`Mail.Ballots.Counted`),
  date      = lubridate::as_date(ymd("2020-10-21"))
)

## append arl totals to vote.data 
vote.data21 <- bind_rows(vote.data21, arl)
...
assert_that((sum(arl$`Mail.Ballots.Requested`[1]) + e.totmail21) == totals.tib$total.mail[1]) # 686 undercount

# sum of total counted == given sum of total counted 
assert_that(sum(arl$`Mail.Ballots.Counted`[1]) == totals.tib$counted[1]) # 136 undercount

# sum of total Mail.Received == given sum of received + given sum of counted 
assert_that( sum(arl$`Mail.Received`[1]) == (totals.tib$received[1] + totals.tib$counted[1] ) ) # 420 undercount

# sum of total outstanding mail votes == given sum of "outstanding" (or "mailed" as imported)
assert_that((sum(arl$`Mail.Outstanding`[1]) + e.outstanding21) == totals.tib$mailed[1]) # 265 undercount

# assert that there are 54 precincts
assert_that(nrow(vote.data21) == nrow )
