...
# page info 
raw.info <- pdf_info(file.path(votes, "10-18-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "10-18-2020.pdf"))
...
vote18 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data18 <-
  left_join(vote18, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
# check math!
assert_that( sum(vote.data18$mail.outstanding.pct) >= 99.5 & sum(vote.data18$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data18$early.voted.pct) >= 99.5 & sum(vote.data18$early.voted.pct) <= 100.5)
assert_that( sum(vote.data18$mail.received.pct) >= 99.5 & sum(vote.data18$mail.received.pct) <= 100.5)
assert_that( sum(vote.data18$prec.tot.votes.pct) >= 99.5 & sum(vote.data18$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data18$prec.early.turnout + vote.data18$prec.mail.turnout) -
                    vote.data18$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data18$`Mail.Ballots.Counted`) / sum(vote.data18$`Mail.Received`)), 1),
...
vote.data18 <- bind_rows(vote.data18, arl)
