# ---  --- --- --- --- --- --- --- --- --- --- --- --- #
# votes-31-oct.R
# imports and processes aggregated vote info from Arlington for 31th october
# ---  --- --- --- --- --- --- --- --- --- --- --- --- #


# load + process voting data file ---- 
# page info 
raw.info <- pdf_info(file.path(votes, "10-31-2020.pdf"))

# import both pages
raw1 <- pdf_text(file.path(votes, "10-31-2020.pdf"))
...
vote31 <- bind_rows(pg1.tib, pg2.tib) %>%
  select(-RawText) %>%
  mutate(
...
# Merge
vote.data31 <-
  left_join(vote31, vote.register,
            by = "precinct",
            suffix = c('.x', '.y')) %>%
  mutate(
...
vote.data31 <-
  ...
  mutate(
    ...
    date     = lubridate::as_date(ymd("2020-10-31"))
  )

# check math!
assert_that( sum(vote.data31$mail.outstanding.pct) >= 99.5 & sum(vote.data31$mail.outstanding.pct) <= 100.5)
assert_that( sum(vote.data31$early.voted.pct) >= 99.5 & sum(vote.data31$early.voted.pct) <= 100.5)
assert_that( sum(vote.data31$mail.received.pct) >= 99.5 & sum(vote.data31$mail.received.pct) <= 100.5)
assert_that( sum(vote.data31$prec.tot.votes.pct) >= 99.5 & sum(vote.data31$prec.tot.votes.pct) <= 100.5)
assert_that( sum( (vote.data31$prec.early.turnout + vote.data31$prec.mail.turnout) -
                    vote.data31$prec.act.turnout > 0.15) == 0)
...
arl <- tibble(
  ...
  `Mail.Outstanding` = sum(vote.data31$`Mail.Outstanding`),
  `Mail.Received` = sum(vote.data31$`Mail.Received`),
  `Early.Voted` = sum(vote.data31$`Early.Voted`),
  `Active.Registered` = sum(vote.data31$`Active.Registered`),
  `Inactive.Registered` = sum(vote.data31$`Inactive.Registered`),
  `All.Registered` = sum(vote.data31$`All.Registered`),
  Military = sum(vote.data31$Military),
  Overseas = sum(vote.data31$Overseas),
  Federal = sum(vote.data31$Federal),
  `Total.Votes` = sum(vote.data31$`Total.Votes`),
  `Outstanding.Votes` = sum(vote.data31$`Outstanding.Votes`),
  `Percent.Outstanding.Votes` = sum(vote.data31$`Percent.Outstanding.Votes`),
  `Mail.Ballots.Requested` = sum(vote.data31$`Mail.Ballots.Requested`),
  `Percent.Mail.Counted`  =  round( 100 *(sum(vote.data31$`Mail.Ballots.Counted`) / sum(vote.data31$`Mail.Received`)), 1),
  `Precinct.Share.of.All.Votes` =  100, # must be 100
  `Mail.Ballot.Return.Rate` = round( (sum(vote.data31$`Mail.Received`) /
                                        sum(vote.data31$`Mail.Ballots.Requested`)), 2),
  ...
  `Mail.Ballots.Counted`  = sum(vote.data31$`Mail.Ballots.Counted`),
  date      = lubridate::as_date(ymd("2020-10-31"))
)

## append arl totals to vote.data 
vote.data31 <- bind_rows(vote.data31, arl)
...
assert_that(sum(arl$`Mail.Ballots.Requested`[1] + e.totmail31) == totals.tib$total.mail[1]) # 101 undercount
...
assert_that((sum(arl$`Mail.Outstanding`[1]) + e.outstanding31) == totals.tib$mailed[1]) # 101 off undercount

# assert that there are 54 precincts
assert_that(nrow(vote.data31) == nrow )
